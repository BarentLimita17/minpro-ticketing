...
const storage = multer.diskStorage({
    destination: (req: any, file: any, cb: any) => {
        ...
        const isDirectoryExist = fs.existsSync(`${defaultDir}/${childDirectoryBasedOnMimetype}`)
        if (isDirectoryExist === false) fs.mkdirSync(`${defaultDir}/${childDirectoryBasedOnMimetype}`, { recursive: true })
...
const fileFilter = (req: any, file: any, cb: any) => {
    const fileAccepter = ['webp', 'jpg', 'jpeg', 'png', 'svg']
    const splitOriginalName = file.originalname.split('.')
    const fileExtension = splitOriginalName[splitOriginalName.length - 1]

    if (!fileAccepter.includes(fileExtension)) return cb(new Error('Only accept webp, jpg, jpeg, png, svg'), false)
    cb(null, true)
}
